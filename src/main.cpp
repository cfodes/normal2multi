...
// mesh_block 类：用于存储单个网格块的信息
class mesh_block {
public:
    idx_t block_id;                         // 分区号，也就是该块所属的分区编号
    double block_D;                         // 分区插值系统所用的D, 对应于DRRBF里的psi = 1-r/D
    ...
    mesh_block() = default; //默认构造

    void set_internal_nodes_df(vector<Node> wall_nodes_0, vector<Node> wall_nodes, unordered_map<int, int>& id2node) //计算网格内部节点的待变形量 = 已知变形量 - 已变形的量
    ...
    void set_blk_nodes_df(const vector<Node>& wall_prev, const unordered_map<int, int>& id2node) //计算网格内部节点的待变形量 = 已知变形量 - 已变形的量
    ...
    void set_block_tree()  //内部节点建立后，构建内部节点的二叉树
    {
...
//全局变量定义：
vector<mesh_block> _blocks;          //网格块vector数组
vector<Node> _unique_boundary_nodes; //物面网格块共享的边界节点，且数组内部元素是唯一的
...
int _search_block_id(const vector<mesh_block>& blocks, const Node inode, double& d_r2omega)  //查找离待插值点最近的分区，返回该分区的id
//该函数遍历所有分区的二叉树，搜索离该点最近的二叉树
//传入参数：分区数组 blocks
//       待插值节点 inode
{
    idx_t id_temp = 0;
    ...
    {
        blocks[k].block_tree.search(d_temp, key, inode.point);   //查找inode在该二叉树最小的距离，返回最小距离d_temp和对应的在blocks.internal_nodes的对应索引
        if (d_temp < d_min)  //判断是否是最小的距离
        {
            d_min = d_temp;
            id_temp = blocks[k].block_id;   //最小的距离对应的分区id
        }
    }
    d_r2omega = d_min;
    return id_temp;
...
GridBTree<int, Point<double>> _set_unique_boundary_nodes_tree(const vector<Node>& unique_boundary_nodes)  //该函数构建一个基于_unique_boundary_nodes的二叉树，这是由于每个分区的二叉树并不包含这些点
...
void _set_block_rbf(vector<deform_calculate>& block_rbf, const vector<mesh_block>& blocks)
//根据网格块组生成每个网格块的rbf插值系统的支撑点集
//支撑点包括：
//该网格块的点
//网格块的block_D形成的圆所涵盖的网格块的点
{
    double d_temp = 0.0;
    ...
    {
        unordered_set<int> nodes_id_set;    //external_suppoints的查找集，确保点是唯一的
        d_temp = blocks[i].block_D;    //设置当前区间block[i]的大小
        for (int j = 0; j < blocks.size(); ++j)  //计算其他区间到block[i]的距离
        {
            for (int k = 0; k < blocks[j].internal_nodes.size(); ++k)
            {
                if (j == i)
                {
                    block_rbf[i].external_suppoints.push_back(blocks[i].internal_nodes[k]);   //将该网格快的内部点点插入支撑点集
                    nodes_id_set.insert(blocks[i].internal_nodes[k].id);             //将该点的id插入查找集里
                }
                else
                {
                    {
                        if (nodes_id_set.find(blocks[j].internal_nodes[k].id) == nodes_id_set.end())
                        {
                            blocks[i].block_tree.search(d_r2omgea_temp, key_temp, blocks[j].internal_nodes[k].point);   //查找第k个区到该区的最小距离
                            if (d_r2omgea_temp <= d_temp)    //该分区被block_D覆盖
                            {
                                block_rbf[i].external_suppoints.push_back(blocks[j].internal_nodes[k]);    //将该区的内部点插入支撑点集
            ...
            for (int k = 0; k < blocks[j].boundary_nodes.size(); ++k)
            {
                blocks[i].block_tree.search(d_r2omgea_temp, key_temp, blocks[j].boundary_nodes[k].point);   //查找第k个区到该区的最小距离
                if (d_r2omgea_temp <= d_temp)    //该分区被block_D覆盖
                if (nodes_id_set.find(blocks[j].boundary_nodes[k].id) == nodes_id_set.end())
                {
                    block_rbf[i].external_suppoints.push_back(blocks[j].boundary_nodes[k]);    //将该区的边界点插入支撑点集
                    block_rbf[i].external_suppoints.back().df.setZero();
                }
            }
        }
    }
